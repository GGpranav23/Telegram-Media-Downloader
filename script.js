// ==UserScript==
// @name         Telegram Media Downloader - Pranav Edition
// @name:en      Telegram Media Downloader - Pranav Edition
// @version      1.0
// @namespace    https://github.com/Pranav-GitHub/Telegram-Media-Downloader
// @description  Download images, GIFs, videos, and voice messages on the Telegram web app from private channels that disable downloading and restrict saving content
// @author       Pranav
// @license      GNU GPLv3
// @website      https://github.com/Pranav-GitHub/Telegram-Media-Downloader
...
(function () {
    ...
    const tel_download_video = async (url) => {
      ...
      try {
        ...
        let supportsRange = headResponse.headers.get('Accept-Ranges') === 'bytes';
        _total_size = parseInt(headResponse.headers.get('Content-Length'));
  
        if (!supportsRange || !_total_size) {
          throw new Error("Server does not support parallel downloads");
        }
  
        const NUM_PARALLEL = 4;
        const chunkSize = Math.ceil(_total_size / NUM_PARALLEL);
        const chunks = Array.from({ length: NUM_PARALLEL }, (_, i) => {
          const start = i * chunkSize;
          const end = Math.min(start + chunkSize - 1, _total_size - 1);
          return { start, end };
        });
  
        const loadedBytesArray = new Array(chunks.length).fill(0);
        ...
        const fetchChunk = async ({ start, end }, index) => {
          ...
          while (true) {
            ...
            loadedBytesArray[index] = receivedLength;
            const totalLoaded = loadedBytesArray.reduce((a, b) => a + b, 0);
            const progress = ((totalLoaded / _total_size) * 100).toFixed(0);
